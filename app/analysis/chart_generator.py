...
def generate_chart(symbol: str, period: str = "1y") -> str:
    ...
    try:
        ...
        ax1.plot(dates, plot_df['MA5'], label='MA5', color='#FF9800', linestyle='--', linewidth=1)
        ax1.plot(dates, plot_df['MA20'], label='MA20', color='#2196F3', linewidth=1.5)
        if 'MA60' in plot_df.columns:
            ax1.plot(dates, plot_df['MA60'], label='MA60', color='#4CAF50', linewidth=1.5, alpha=0.7)

        ax1.fill_between(dates, plot_df['BB_Upper'], plot_df['BB_Lower'], alpha=0.1, color='gray', label='Bollinger')
        ...
        ax2.plot(dates, plot_df['Volume_MA5'], color='orange', linewidth=1, label='Vol MA5')
        ...
        ax3.plot(dates, plot_df['RSI'], color='purple', linewidth=1, label='RSI(14)')
        ax3.axhline(70, color='red', linestyle='--', alpha=0.5)
        ax3.axhline(30, color='green', linestyle='--', alpha=0.5)
        ax3.fill_between(dates, plot_df['RSI'], 70,
                         where=(plot_df['RSI'] >= 70), facecolor='red', alpha=0.3)
        ax3.fill_between(dates, plot_df['RSI'], 30,
                         where=(plot_df['RSI'] <= 30), facecolor='green', alpha=0.3)
        ax3.set_ylabel("RSI")
        ax3.set_ylim(0, 100)
        ...
        macd_colors = ['#4CAF50' if v >= 0 else '#F44336' for v in plot_df['MACD_Histogram']]
        ax4.bar(dates, plot_df['MACD_Histogram'], color=macd_colors, alpha=0.7, width=0.8)
        ax4.plot(dates, plot_df['MACD_DIF'], color='blue', linewidth=1, label='DIF')
        ax4.plot(dates, plot_df['MACD_DEA'], color='orange', linewidth=1, label='DEA')
